  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: TabBarView(
        controller: _tabController,
        children: _tabs
            .map((tab) => _OrderList(tab: tab, searchQuery: _searchQuery))
            .toList(),
      ),
    );
  }
    ...
    return StreamBuilder(
      ...
      builder: (context, snapshot) {
        ...
        if (tab.statusFilter != null) {
          allOrders = allOrders
              .where((order) => tab.statusFilter!.contains(order.status))
              .toList();
        }
...
class _OrderCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Semantics(
      label: 'btn_order_card_${order.orderId}',
